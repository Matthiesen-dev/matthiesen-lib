...
public class MatthiesenLibClient {
     ...
     * @param menuTypeSupplier A supplier that provides the MenuType associated with the screen. This allows for lazy evaluation, so you can pass a reference to a menu type that may not be initialized yet.
    ...
    /**
     * A record to store information about a screen registration, including the menu type and the screen constructor. This is used to queue screen registrations before they are applied to the game, allowing for lazy evaluation of menu types and constructors. The apply method is used to register the screen with the provided ScreenRegistrar when the platform calls for it.
     *
     * @param menuType The MenuType associated with the screen. This is used to identify which menu the screen should be opened for, and it should match the menu type that is used for the screen's container.
     * @param screenConstructor The constructor for the screen, which takes the menu and the player's inventory as parameters. This is used to create the screen instance when the menu is opened.
     * @param <M> The menu type for the screen. This should be a subclass of AbstractContainerMenu, and it should match the menu type that is used for the screen's container. This allows the screen to access the menu's inventory and other data as needed.
     * @param <S> The screen type, which must implement MenuAccess<M>. This allows the screen to access the menu's inventory and other data as needed, and it ensures that the screen is compatible with the menu type that it is associated with.
     */
    private record ScreenEntry<M extends AbstractContainerMenu, S extends Screen & MenuAccess<M>>(
        ...
        /**
         * Registers the screen with the provided ScreenRegistrar. This should be called during the platform's screen registration event, and it will register the screen with its associated menu type so that it can be opened when the menu is opened.
         *
         * @param registrar The ScreenRegistrar provided by the platform, which is used to register screens with their associated menu types. This should be called during the platform's screen registration event, and it will register the screen with the game so that it can be opened when the associated menu is opened.
         */
